...
# D3 Graph
The D3 graph gets all of the json data into an 100 length array of notes, that stores all the instruments primary information such as duration of a note, post gain, etc... What my d3 graph does is it displays the postgain of all the different instruments at once, since there are only 4 for this assessment it is quite basic and doesn't jump around alot, yet with some modifications made to the songText and how many instruments there are the D3 graph would be displaying all the different levels of postgain that each instrument in playing when the instrument plays there notes.


# CANVAS
Canvas simply is a piano roll of all the different instruments and there notes playing. Was already implemented prior to me working on this assessment and wasn't modified as apart of my code. Just adjusted its position within the UI